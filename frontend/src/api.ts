...
export const api = {
  ...
  problemTimeline: (id: string) => request<{ cluster_id: string; items: import("./types").TimelineItem[] }>(`/problems/${id}/timeline`),
  problemWork: (id: string) => request<{ cluster_id: string; commitments: import("./types").ResponseCommitment[]; project_tasks: import("./types").ProjectTask[] }>(`/problems/${id}/work`),
  ...
  addReportFollowUpEvidence: (id: string, payload: Record<string, unknown>) => request<any>(`/problem-reports/${id}/follow-up-evidence`, { method: "POST", body: JSON.stringify(payload) }),
  requestReportCorrection: (id: string, reason: string) => request<any>(`/problem-reports/${id}/request-correction`, { method: "POST", body: JSON.stringify({ reason, decision: "REQUEST_CORRECTION" }) }),
  withdrawReport: (id: string) => request<any>(`/problem-reports/${id}/withdraw`, { method: "POST" }),
  ...
  assessPriority: (id: string, payload: Record<string, unknown>) => request<import("./types").Cluster>(`/problems/${id}/priority-assessment`, { method: "POST", body: JSON.stringify(payload) }),
  ...
  work: (query = "") => request<{ items: import("./types").WorkItem[]; summary: Record<string, number>; manager_view: boolean }>(`/work${query}`),
  createCommitment: (payload: Record<string, unknown>) => request<import("./types").ResponseCommitment>("/response-commitments", { method: "POST", body: JSON.stringify(payload) }),
  updateCommitment: (id: string, payload: Record<string, unknown>) => request<import("./types").ResponseCommitment>(`/response-commitments/${id}`, { method: "PATCH", body: JSON.stringify(payload) }),
  addCommitmentUpdate: (id: string, payload: Record<string, unknown>) => request<import("./types").ResponseCommitment>(`/response-commitments/${id}/updates`, { method: "POST", body: JSON.stringify(payload) }),
  transferCommitment: (id: string, payload: Record<string, unknown>) => request<import("./types").ResponseCommitment>(`/response-commitments/${id}/transfer`, { method: "POST", body: JSON.stringify(payload) }),
  completeCommitment: (id: string, payload: Record<string, unknown>) => request<import("./types").ResponseCommitment>(`/response-commitments/${id}/complete`, { method: "POST", body: JSON.stringify(payload) }),
