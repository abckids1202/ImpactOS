...
export function MemberLogin({ user, onLogin }: { user: User | null; onLogin: (user: User | null) => void }) {
  ...
  return <AuthLayout eyebrow="SPI MEMBER ACCESS" title="Welcome back." description="Sign in to continue your projects, contribute evidence, or review assigned work."><form className="auth-form" onSubmit={submit}><label>Email<input required type="email" autoComplete="username" value={email} onChange={(event) => setEmail(event.target.value)} /></label><label>Password<input required type="password" autoComplete="current-password" value={password} onChange={(event) => setPassword(event.target.value)} /></label><button className="public-button primary auth-submit" disabled={busy}>{busy ? "Signing in…" : "Sign in"}</button></form><AuthError error={error} /><div className="auth-links"><Link to="/forgot-password">Forgot password?</Link><span>New member?</span><Link to="/activate">Activate an SPI account</Link></div>{demo && <div className="demo-access"><button className="demo-toggle" onClick={() => setShowDemo((value) => !value)} type="button">Developer demo access</button>{showDemo && <div className="demo-list">{["student@demo.local", "leader@demo.local", "mentor@demo.local", "osis@demo.local", "moderator@demo.local", "admin@demo.local"].map((demoEmail) => <button type="button" key={demoEmail} onClick={() => fillDemo(demoEmail)}>{demoEmail}</button>)}</div>}</div>}</AuthLayout>;
}
...
export function InvitePage({ onLogin }: { onLogin: (user: User | null) => void }) {
  ...
  const submit = async (event: FormEvent) => { event.preventDefault(); setBusy(true); setError(null); try { const result = await api.acceptInvitation(token, { display_name: displayName.trim(), password }); onLogin(result.user); navigate(DEFAULT_MEMBER_PATH, { replace: true }); } catch (err) { setError(err); } finally { setBusy(false); } };
